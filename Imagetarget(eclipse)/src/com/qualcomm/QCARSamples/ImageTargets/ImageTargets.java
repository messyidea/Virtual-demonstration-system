...
public class ImageTargets extends Activity
{
    ...
    private void loadTextures()
    {
       /* mTextures.add(Texture.loadTextureFromApk("TextureTeapotBrass.png",
        ...
        mTextures.add(Texture.loadTextureFromApk("TextureTeapotRed.png",
                getAssets()));*/
        /*mTextures.add(Texture.loadTextureFromApk("teapot1.jpg",
                getAssets()));
        mTextures.add(Texture.loadTextureFromApk("teapot2.jpg",
                getAssets()));
        mTextures.add(Texture.loadTextureFromApk("teapot3.jpg",
                getAssets()));*/
    	mTextures.add(Texture.loadTextureFromApk("teapot.png",
                getAssets()));
        mTextures.add(Texture.loadTextureFromApk("heroin.bmp",
                getAssets()));
   ...
   /** Called when the activity will start interacting with the user.*/
    ...
    private native int getmodelmessage();
    ...
    private void showCameraOptionsDialog()
    {
        ...
        {
            items[itemSwitchDatasetIndex] = ("about the model");
        }
        else
        {
            items[itemSwitchDatasetIndex] = ("about the model");
                ...
                new DialogInterface.OnClickListener()
                {
                    public void onClick(DialogInterface dialog, int item)
                    {
                        ...
                        else if (item == 2)
                        {
                            ...
                            //dialog.dismiss();
                        	//String a = String.valueOf(getmodelmessge());
                        	//System.out.println(a);
                        	dialog.dismiss();
                        	new AlertDialog.Builder(ImageTargets.this)
    ...
    public String getmodel(){
    	int temp = getmodelmessage();
    	if(temp == 1){
    		return "海洛因，来源于鸦片，是吗啡二乙酰的衍生物，其化学名为二乙酰吗啡。";
    	}
    	else if(temp == 2){
    		return "这是灰太狼";
    	}
    	else if(temp == 3){
    		return "甲烷在自然界的分布很广，甲烷是最简单的有机物，俗名：天然气，沼气。";
    	}
    	else if(temp == 4){
    		return "氢气是世界上已知的密度最小的气体。";
    	}
    	else if(temp == 5){
    		return "二氧化碳是空气中常见的化合物，一个二氧化碳分子由两个氧原子与一个碳原子通过共价键构成";
    	}
    	else return "暂时没有捕捉到标志物";
    }
}
